...
import ai.turbochain.ipex.config.AliyunConfig;
import ai.turbochain.ipex.fdfs.FastDFSClient;
import ai.turbochain.ipex.fdfs.FastDFSFile;
import ai.turbochain.ipex.service.LocaleMessageSourceService;
import ai.turbochain.ipex.util.GeneratorUtil;
import ai.turbochain.ipex.util.MessageResult;
import ai.turbochain.ipex.util.UploadFileUtil;
...
public class UploadController {
    ...
    @Value("${fastdfs.img.host}")
    private String fastdfsImgHost;
    @Autowired
     ...
     * @throws IOException
     */
    @RequestMapping(value = "upload/dfs/image", method = RequestMethod.POST)
    @ResponseBody
    public MessageResult uploadDfsImage(HttpServletRequest request, HttpServletResponse response,
    ...
    @RequestMapping(value = "/upload/fastdfs/image", method = RequestMethod.POST)
    @ResponseBody
    public String uploadFastdfsImage(HttpServletRequest request,@RequestParam("file") MultipartFile file) {
        ...
        try {
            ...
            if (fileAbsolutePath==null) {
                ...
                return MessageResult.error(500, "上传失败!").toString();
            }
            //FastDFSClient.getTrackerUrl()
            String path = fastdfsImgHost + fileAbsolutePath[0]+ "/"+fileAbsolutePath[1];
